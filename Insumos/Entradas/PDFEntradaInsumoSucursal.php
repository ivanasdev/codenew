<?php
	...
	class PDF extends PDF_MC_Table{
		function Header(){
			...
			$titulo = "DETALLE ENTRADA DE INSUMOS";
			$tituloFolio = "FOLIO";
			$idFolio = $_GET['idCabeceraEntrada'];
			...
			WHERE id_CabeceraEntrada = '".$_GET['idCabeceraEntrada']."'";
			$res = mssql_query($quer);
			$row = mssql_fetch_object($res);

			//$this->Cell(15,5,'PROVEEDOR: ',0,0,'L');
			$this->SetX(130);
			$this->Cell(25,5,'MOVIMIENTO: Entrada de Insumos',0,0,'L');
	...
	$pdf->Cell($widths[5],3,'Observaciones',1,0,'C',1);
	...
	FROM tbl_SUCEntradaInsumoDetalle t1 
	...
	$query0 = "SELECT st_Observaciones FROM tbl_SUCEntradaInsumo WHERE id_CabeceraEntrada = '".$idCabeceraEntrada."'";
	...
	$pdf->Output('DetalleEntradaInsumo_'.$idCabeceraEntrada.'.pdf','D');
	mssql_close();
?>