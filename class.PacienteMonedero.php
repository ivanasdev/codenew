...
function obtenSaldoMonederoJOJUTLA($idUsuarioWeb){

	$query0 = "SELECT i_Saldo FROM tbl_MonederoUsuarioWeb WHERE id_Monedero = '2' AND id_UsuarioWeb = '".$idUsuarioWeb."'";
	$rquery0 = mssql_query($query0);
	$arrayQuery0 = mssql_fetch_array($rquery0);
	$saldo = $this->truncateFloat($arrayQuery0["i_Saldo"],2);	
	return $saldo;
...
function esPacienteJOJUTLA($idUsuarioWeb){
	
	$query0 = "SELECT COUNT(*) as conteo FROM cat_JOJEmpleados 
	WHERE id_UsuarioWeb != '0' AND id_UsuarioWeb = '".$idUsuarioWeb."' AND i_Activo = '1'";
	$rquery0 = mssql_query($query0);
	$arrayQuery0 = mssql_fetch_array($rquery0);
	$conteo = $arrayQuery0["conteo"];
